...
sortFiles Command{ opt_sort_order    = sort_order     -- порядок сортировки в виде строки, например "gen" - сортировать по группе, расширению и имени
    ...
    -- 4. выводим группы в порядке возрастания размеров группы? среднего размера файлов в группе?
      ...
      where (o1,'i':o2) =  break (=='i') sortOrder
            isort group = -- Отсортировать список файлов, имеющих одинаковое расширение
              let groups = groupOn three (sortOn (keyFunc o2 find_group) group)  -- сгруппировать по первым 3-м буквам имени
   ...
   _ | null main_list   -> return added_list  -- если один из списков пуст, то просто возвращаем второй
    ...
    let select_file arcfile = do
          ...
          case (diskfile, update_type) of
            (Nothing, 's')        ->  return Nothing        -- Файла с таким именем на диске нет: режим "
            ...
            (Just diskfile, 's')  ->  return$ Just (sync_file  arcfile diskfile)  -- режим "
    ...
    let mergeFunction = case () of     -- Добавить новые файлы в конец архива в двух случаях:
          _ | append         -> (++)   --    при установленной опции "
...
splitToSolidBlocks command filelist  =  (dirs &&& [(aNO_COMPRESSION,dirs)])
  ...
  where
    ...
    -- В командах копирования архива и при
...
aGroupSize = 2*mb       -- общий список файлов разбивается на куски этого размера, для каждого куска мы сначала пытаемся определить общий тип
aCHUNKS = 5              -- кол-во проб, которые делаются для определения типа файлов
aChunkSize = 64*kb      -- размер каждой пробы
 ...
 where                           -- todo: добавить разбивку по группам
  ...
  groupType [file] = do
    let defaultType = getDefaultType file             -- тип файла по arc.groups
        ...
        chunks = filesize `div` aChunkSize + 1       -- на сколько блоков *можно* разделить файл
        n = if chunks < aCHUNKS  then chunks          -- количество проверяемых блоков
                                 else sqrt (i$ aCHUNKS * chunks) .$round
        blocksize = min aChunkSize (filesize `div` n)  -- размер проверяемых блоков
